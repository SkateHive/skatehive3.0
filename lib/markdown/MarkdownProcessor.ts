...
export interface VideoPlaceholder {
  type: 'VIDEO' | 'ODYSEE' | 'YOUTUBE' | 'VIMEO' | '3SPEAK' | 'SNAPSHOT' | 'SKATEHIVEGAME' | 'BUILDERPROPOSAL' | 'POIDHBOUNTY' | 'PREDICTIONMARKET';
  id: string;
  placeholder: string;
}
...
export class MarkdownProcessor {
  ...
  private static extractVideoPlaceholders(content: string): VideoPlaceholder[] {
    ...
    const regex = /\[\[(VIDEO|ODYSEE|YOUTUBE|VIMEO|3SPEAK|SNAPSHOT|SKATEHIVEGAME|BUILDERPROPOSAL|POIDHBOUNTY|PREDICTIONMARKET):([^\]]+)\]\]/g;
    let match;
